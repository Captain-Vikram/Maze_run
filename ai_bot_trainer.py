...
def play_game_with_training():
    ...
    grid_sizes = [(x, x) for x in range(11, 31)]  # 11x11 to 30x30

    for grid_size in grid_sizes:
        print(f"Training on grid size: {grid_size[0]}x{grid_size[1]}")
        
        # Initialize the game with the current grid size
        game = AdaptiveMazeGame("BotPlayer")
        
        # Set maze parameters explicitly before generating the maze
        game.maze_params = {"width": grid_size[0], "height": grid_size[1]}
        
        # Reset level for each new grid size
        current_level = 1
        performance_metrics = []
        ...
        for iteration in range(100):
            ...
            # Generate a new maze with the specified grid size
            while True:
                game.generate_maze()
            ...
            # Debugging: Confirm maze generation with correct grid size
            print(f"Generated new maze for grid size: {grid_size[0]}x{grid_size[1]}")
            
            # Initialize the bot for this specific grid size
            bot = EnhancedMazeBot(game, level=current_level)
            
            # Run the bot until it solves the maze
            result = None
            while bot.state != bot.goal and result != "regenerate":
                result = bot.step()
                if result == "regenerate":
                    game.generate_maze()  # Regenerate the maze
                    bot = MazeBot(game, game.current_level)  # Reset the bot
                    print("Maze regenerated.")
            
            # Collect performance data if the maze was solved
            if bot.state == bot.goal:
                performance_data = bot.get_performance_data()
            ...
            # Save Q-table periodically for the specific grid size
            if iteration % 10 == 0:
                bot.agent.save_q_table(current_level)
                current_level += 1  # Increment level counter for saving progress
    ...
    print("Training completed for all grid sizes.")
